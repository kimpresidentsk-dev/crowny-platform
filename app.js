...
async function showSendModal() {
    ...
    if (contacts.empty) {
        ...
        await sendTokensByEmail(email, parseFloat(amount));
    } else {
        ...
        const amount = prompt(`${recipientEmail}에게 전송할 CRNY 수량:`);
        if (!amount) return;
        
        await sendTokensByEmail(recipientEmail, parseFloat(amount));
    }
}

async function sendTokensByEmail(recipientEmail, amount) {
    if (!userWallet) return;
    
    if (amount <= 0 || amount > userWallet.balances.crny) {
        alert('잔액이 부족하거나 잘못된 수량입니다');
        return;
    }
