        ...
        for (const doc of challenges.docs) {
            ...
            card.innerHTML = `
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                    <div>
                        <strong>참가비:</strong> ${challenge.entryFee} CRNY
                    </div>
                    <div>
                        <strong>목표 수익률:</strong> ${challenge.targetProfit}%
                    </div>
                    <div>
                        <strong>참가자:</strong> ${challenge.participants || 0}명
                    </div>
                    <div>
                        <strong>기간:</strong> ${challenge.duration}일
                    </div>
                </div>
                <button onclick="joinChallenge('${doc.id}')" class="btn-primary" style="width:100%;">
                    🚀 챌린지 참가
                </button>
            `;
...
async function showCreateChallenge() {
    ...
    const name = prompt('챌린지 이름:');
    if (!name) return;
    
    const description = prompt('설명:');
    const entryFee = parseFloat(prompt('참가비 (CRNY):'));
    const targetProfit = parseFloat(prompt('목표 수익률 (%):'));
    const duration = parseInt(prompt('기간 (일):'));
...
async function joinChallenge(challengeId) {
    ...
    const confirm = window.confirm(
        ...
        `목표: ${data.targetProfit}% 수익\n` +
        `기간: ${data.duration}일\n\n` +
        `✅ 참가비는 트레이딩 상금 풀로 이동합니다`
    );
...
function initTradingViewChart() {
    ...
    // TradingView 차트가 준비되면 실행
    widget.onChartReady(() => {
        const chart = widget.activeChart();
        window.tvChart = chart; // 전역 변수로 저장
        
        console.log('📊 TradingView 차트 준비 완료');
        
        // 가격 업데이트
        setTimeout(() => updatePriceFromChart(chart), 2000);
        
        // 오픈 포지션이 있으면 차트에 라인 그리기
        setTimeout(() => drawPositionLines(), 3000);
    });
    
    return widget;
}

// 차트에 포지션 라인 그리기 (진입가, SL, TP)
function drawPositionLines() {
    if (!window.tvChart || !myParticipation || !myParticipation.trades) return;
    ...
    openTrades.forEach((trade, index) => {
        ...
        const entryLine = chart.createShape(
            { time: Date.now() / 1000, price: trade.entryPrice },
            {
                shape: 'horizontal_line',
                overrides: {
                    linecolor: trade.side === 'BUY' ? '#0066cc' : '#cc0000',
                    linewidth: 2,
                    linestyle: 0,
                    showLabel: true,
                    text: `${trade.side} ${trade.contracts} @ ${trade.entryPrice.toFixed(2)}`
                }
            }
        );
        ...
        if (trade.stopLoss) {
            const slLine = chart.createShape(
                { time: Date.now() / 1000, price: trade.stopLoss },
                {
                    shape: 'horizontal_line',
                    overrides: {
                        linecolor: '#ff0000',
            ...
            // 라인 이동 가능하게 설정
            slLine.setMoveable(true);
            
            // 라인 이동 시 이벤트
            slLine.onMove().subscribe(null, (price) => {
                updateTradeStopLoss(actualIndex, price);
            });
        ...
        if (trade.takeProfit) {
            const tpLine = chart.createShape(
                { time: Date.now() / 1000, price: trade.takeProfit },
                {
                    shape: 'horizontal_line',
                    overrides: {
                        linecolor: '#00cc00',
            ...
            tpLine.setMoveable(true);
            
            tpLine.onMove().subscribe(null, (price) => {
                updateTradeTakeProfit(actualIndex, price);
            });
...
async function executeFuturesTrade(side) {
    ...
    const multiplier = contract === 'NQ' ? 20 : 2;
    const margin = contract === 'NQ' ? 15000 : 1500; // 증거금 (예시)
    ...
    try {
        ...
        alert(`✅ ${side} 포지션 진입!\n${contract} ${contracts}계약 @ ${currentPrice.toFixed(2)}`);
        updateTradingUI();
        updateOpenPositions();
        loadTradeHistory();
    } catch (error) {
...
async function closePosition(tradeIndex) {
    ...
    try {
        ...
        const newBalance = myParticipation.currentBalance + trade.margin + pnl;
        ...
        // 차트 라인 업데이트
        setTimeout(() => drawPositionLines(), 500);
    } catch (error) {
...
// Modify executeFuturesTrade to support advanced order types
async function executeFuturesTrade(side) {
    if (!myParticipation) {
        alert('챌린지에 먼저 참가하세요');
        return;
    ...
    try {
        ...
        // 차트에 라인 그리기
        setTimeout(() => drawPositionLines(), 1000);
    } catch (error) {
        alert('거래 실패: ' + error.message);
    }
}

// Quick chart trade (1 contract market order with default SL/TP)
async function quickChartTrade(side) {
    ...
    try {
        ...
        console.log(`✅ 차트 ${side} 주문 체결! SL: ${stopLoss.toFixed(2)}, TP: ${takeProfit.toFixed(2)}`);
