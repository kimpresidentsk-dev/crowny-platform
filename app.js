...
function updateTradingUI() {
    ...
    document.getElementById('trading-profit').style.color = profit >= 0 ? 'green' : 'red';
...
function initTradingViewChart() {
    new TradingView.widget({
        "autosize": true,
        "symbol": "CME_MINI:NQ1!", // 나스닥100 E-mini 선물
        "interval": "5",
        "timezone": "America/Chicago", // CME 시카고 시간
        "theme": "light",
        "style": "1",
        "locale": "kr",
        "toolbar_bg": "#f1f3f6",
...
async function updateNQPrice() {
    try {
        // 실제로는 유료 API 필요 (CME 데이터)
        // 임시: 랜덤 변동 (실제 NQ 가격 범위: 19,000 - 21,000)
        if (!currentPrice) {
            currentPrice = 20500; // 시작 가격
        } else {
            // -50 ~ +50 포인트 랜덤 변동
        ...
        const multiplier = contract === 'NQ' ? 20 : 2; // NQ=$20/pt, MNQ=$2/pt
        const contractValue = (currentPrice * multiplier).toFixed(2);
        
        document.getElementById('current-nq-price').textContent = currentPrice.toFixed(2);
        document.getElementById('contract-value').textContent = `$${contractValue}`;
...
function updateOpenPositions() {
    ...
    openTrades.forEach((trade, index) => {
        ...
        div.style.cssText = 'padding:1rem; background:var(--bg); border-radius:6px; margin-bottom:0.5rem; border-left:4px solid ' + (trade.side === 'LONG' ? 'green' : 'red');
        
        div.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    <strong style="color:${trade.side === 'LONG' ? 'green' : 'red'}">${trade.side}</strong> 
                    ${trade.contract} × ${trade.contracts}
                    <br>
                ...
                <button onclick="closePosition(${actualIndex})" style="background:var(--accent); color:white; border:none; padding:0.5rem 1rem; border-radius:6px; cursor:pointer;">
                    청산
                </button>
            </div>
        `;
