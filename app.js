...
async function loadUserWallet() {
    ...
    if (doc.exists) {
        ...
        // Wallet type
        const walletType = userWallet.isImported ? '📥 외부 지갑' : '🏠 크라우니 지갑';
        ...
        if (!userWallet.isImported) {
            ...
            const totalGas = userWallet.totalGasSubsidy || 0;
...
function showImportWallet() {
    ...
    try {
        ...
        if (confirm) {
            importExternalWallet(privateKey, account.address);
        }
    } catch (error) {
        alert('잘못된 개인키입니다');
    }
...
async function importExternalWallet(privateKey, address) {
    try {
